...
const Emortion = (props) => {
            ...
            return (<div>{
                emortion.comments.map((comment, index) => {
                    return (
                        ...
                        <Comment getComments={getComments} key={index} comment={comment} postId={emortion._id}
                                 getPosts={props.getPosts} user={props.user}/>
                    )
                })}</div>)
        } else {
    ...
    function RenderEmojis() {
        if (emortion.message.emojiObjects != null & emortion.message.emojiObjects.length > 0) {
            return (
                emortion.message.emojiObjects.map((emojidata, index) => (
                    <Emoji key={index} emoji={emojidata} size={28}/>
                ))
            )
        } else if (emortion.message.emojiArray != null) {
            return (
